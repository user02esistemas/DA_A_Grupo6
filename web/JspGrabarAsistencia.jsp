    ...
    <body>
        ...
        try {
            ...
            if (idEmpParam != null && !idEmpParam.trim().isEmpty()) {
                ...
                Empleados emp = asistControl.buscarEmpleadoPorId(Integer.parseInt(idEmpParam.trim()));
                if (emp == null) {
                    throw new Exception("El empleado seleccionado no existe.");
                }

                asistencia.setIdEmpleado(emp);
                asistencia.setObservaciones(observacionesParam != null ? observacionesParam.trim() : null);

                asistControl.insertarAsistencia(asistencia);
        %>
                <div class="success-card">
                    <div class="success-icon">✓</div>
                    <h1>¡Entrada registrada!</h1>
                    <p>La asistencia del empleado se ha guardado correctamente en la base de datos.</p>
                    <a href="indexAsistenciaPersonal.jsp" class="btn-back">Volver al Inicio</a>
                </div>
        <%
            } else {
                throw new Exception("Error: Debe seleccionar un empleado.");
            }
        } catch (Exception e) {
        %>
            <div class="success-card" style="border-top: 4px solid #ef4444;">
                ...
                <a href="indexAsistenciaPersonal.jsp" class="btn-back" style="background-color: #dc2626;">Intentar de Nuevo</a>
            </div>
        <%
        }
        %>
